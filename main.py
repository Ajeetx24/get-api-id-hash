...
async def message_handler(_, message: Message):
    ...
    if text.lower() == "/start":
        ...
        get_phone = await app.ask(message.chat.id, "⚙️ Send your phone number or share your phone number:", reply_markup=cancel_keyboard, reply_to_message_id=message.id or 0, filters=filters.text | filters.contact)
        ...
        if phone_number.lower() == "《 cancel 》":
            ...
            if hash:
                get_code = await app.ask(message.chat.id, "⚙️ Forward or copy that message or just send code here:", reply_markup=cancel_keyboard, reply_to_message_id=message.id or 0, filters=filters.text)
                if get_code.text.lower() == "《 cancel 》":
                    ...
                    if token:
                        ...
                        if api:
                            await message.reply(f"🔖 Phone Number: `{phone_number}`\n🔖 API ID: `{api[0]}`\n🔖 API HASH: `{api[1]}`\n📰 Creator: @Staliox", reply_markup=main_keyboard, quote=True)
                        else:
                            await message.reply("⚙️ Cannot get data from telegram!", reply_markup=main_keyboard, quote=True)
                    else:
                        await message.reply("⚙️ Code is invalid!", reply_markup=main_keyboard, quote=True)
            else:
                await message.reply("⚙️ Cannot send code to this number!", reply_markup=main_keyboard, quote=True)
